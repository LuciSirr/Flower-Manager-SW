...
typedef struct
{
  uint8_t  Temp_h;
  uint8_t  Temp_l;
  uint16_t TempRaw;				// *0.0625 to *C
  uint8_t  Temperature;			 
  uint8_t  Temperature_Dec;		// Decimals of temperature result

}ds18b20_t;

//functions
extern uint16_t ds18b20_gettemp();						// Contains wait loop for result
extern uint8_t DS18b20_GetTemp(ds18b20_t *sensor);		// Result provided in next cycle
